...
reg1 <- glm(results_numeric ~ .,
                  data=inspectionTrain %>% st_drop_geometry() %>% dplyr::select(risk, inspection_date, inspection_type, violations_count, past_inspect, past_failed_inspect, no_of_days, sanitation_nn1, sanitation_nn2, sanitation_nn3, sanitation_nn4, sanitation_nn5, rodents_nn1, rodents_nn2, rodents_nn3, rodents_nn4, rodents_nn5, sec_neigh, results_numeric, Temperature, Precipitation, Wind_Speed),
...
reg2 <- glm(results_numeric ~ .,
                  data=inspectionTrain %>% st_drop_geometry() %>% dplyr::select(risk,  violations_count, past_inspect, past_failed_inspect, no_of_days, sanitation_nn4, rodents_nn4, sec_neigh, results_numeric, Temperature),
                  family="binomial" (link="logit"))
summary(reg2)
```
...
cvFit.reg1 <- train(results_numeric ~ ., data = inspections16 %>% st_drop_geometry() %>%
                                   dplyr::select(rodents_nn5, sanitation_nn5, violations_count, risk, results_numeric) %>%
