...
The predicted inspection score can be incorporated into restaurant reviews on sites such as Yelp, Google Maps, and food blogs to enhance awareness of the food safety of restaurants. For more information on Eat Safe, check out this promo video: https://youtu.be/3b2D764zBpU

The model developed here utilizes logistic regression, determining the likelihood of failing inspection and allowing us to classify a restaurant as likely to "fail" or "pass" their next inspection. This allows for clear messaging in the app and through the API that a restaurant is "sure safe" or that a restaurant is potentially "at risk of failing inspection". 

Data used in this analysis came from the [Chicago Data Portal](https://data.cityofchicago.org/). The inspection data contains inspections of restaurants and other food establishments in Chicago since 2010, with this analysis focusing specifically on restaurants. Inspections are performed by staff from the Chicago Department of Public Health’s Food Protection Program using a standardized procedure. Restaurants must be inspected annually but can be inspected multiple times a year if there are complaints or a need for re-inspection, and all of these inspections are included in the model development. However, restaurants that are out of business or indicated they were not ready at time of inspection are not included in the model. Other open data we utilize include Chicago neighborhoods, 311 requests, and liquor licenses. This allows the model to account for spatial features to identify the risk of failing an inspection. Other data included in this analysis includes census data from the American Community Survey. We also use weather data using the riem package in R, which allows us to pull weather data from stations across the world. Common violations for restaurants are due to improper temperature control for food, so the model also incorporates temperature. Using this data, features are engineered, and those features with the most predictive power are included in the final model for Eat Safe.

This document will walk through the model setup, feature engineering, exploratory analysis, and model development and performance behind the new app and API, Eat Safe. 

# Setup

First, we load R packages, functions, and the inspection data. 

```{r setup_13, include=FALSE}
library(tidyverse)
library(sf)
library(lubridate)
library(tigris)
...
Using the inspection dataset, we create variables for number of violations,  number of past inspections, number of past failed inspections, number of days since inspection, days operating, number of critical violations, and if the restaurant is designated "high risk". We add spatial features like neighborhood, distance to santation and rodent complaints, and distance to liquor licenses. We add features from the census by tract, including percent of white population and median income. We also add weather data like wind speed, precipitation, and temperature. 

We also re-code our outcome variable so that it is binary with only two outcomes, pass(0) or fail(1). We coded those inspections where there was no entry for the inspection but violations existed, as a "fail", but coded inspections where there was no entry but no violations as a "pass". If a restaurant passed with conditions, it was considered a "pass" in this analysis.
...
```{r use_nn_function, message=FALSE, warning=FALSE}
...
```{r import_weather_18, message = FALSE, warning = FALSE, cache = TRUE}
...
The risk category of each facility refers to how the establishment is categorized for its risk of adversely affecting the public's health, with 1 being the highest and 3 the lowest (these are establishments that only have pre-packaged food items). It can be seen that those restaurants in the "Risk 1 (High)" category are most likely to fail the inspection.
...
As seen below, restaurants that fail inspection seem to have fewer past inspections and past failed inspections, more critical violations and number of violations overall, and more days since last inspection on average. Restaurants that fail inspection seem to have slightly higher temperatures on the day they fail, lower proportion of white people in their census tract, and a slightly lower income context. 
...
Next we look at those restaurants that failed inspection across Chicago in 2016. It can be seen that the majority of failed inspections occurred in downtown and the northeast part of the city. The neighborhoods that had the most restaurants failing inspection were Lake View, Wicker Park/ West Town, and the Loop.
...
```{r 2016_inspections_by_neighborhood, message=FALSE, warning=FALSE}
...
Further, when we pull in census tract level data, we can see that the northeast part of the city where the most inspections occur is also home to Chicago's lower income census tracts.
...
Looking at the yearly weather trends, we can see that wind speeds vary, but precipitation and temperature increase in spring and summer. As shown abover, the likelihood of failing an inspection increases slightly with higher temperatures. 
...
The number of inspections done each week seems to vary randomly across the year and hovers around 200 inspections a week. 
...
Next, we build the model. The model developed here utilizes logistic regression, determining the likelihood of failing inspection and allowing us to classify a restaurant as likely to "fail" their next inspection or "pass" their next inspection. 

We split our data into a train and test set, and, first, we look a logistic regression model with all of our features included. 
...
Next, we create a better fitting model with fewer, more significant features. In this model, we include the following selected features: high_risk, inspection_type, critical_violations, violations_count, days_operating, Temperature, liquor_nn5, sec_neigh, results_numeric.  
...
A confusion matrix is produced to show the goodness of fit of the model. A positive parameter is specified to let the function know that a value of 1 designates failing inspection. Our sensitivity (true positive rate) is 0.72, which greatly improved after our feature engineering. Our specificity (true negative rate) is high at 0.87. Accuracy is also high at 0.84. An optimal threshold of 0.29 was used to produce this confusion matrix (see below for development of this optimal threshold).
...
We create a table to show the confusion matrix results.  
...
Next, we look at how our model built on 2016 data performs on the next year's data. We find the model still performs well, with only a small decrease in accuracy and some decreased sensitivity but increased specificity. 
...
Looking at the predictions we got incorrect in 2017 (incorrect prediction = 1), we can see they are fairly evenly distributed across space, with the bulk appearing in downtown and the northeast part of the city (where we see the most inspections and failed inspections). 
...
Next, we look at how our model built on 2016 data performs on 2018 data. The performance of the model is decreasing. 
...
Despite these limitations, the model developed and analyzed here is fairly accurate and generalizes well. The model behind the the Eat Safe app will also be reanalyzed and developed each year using updated data, which should improve the accuracy of predictions in future years and better protect the public and restaurants. We believe this model can help the public make an informed decision about where they eat, which is especially important to maintain hygiene standards during the pandemic! 
