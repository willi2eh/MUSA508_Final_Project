...
# Map of predictions
inspections17 <- inspections17 %>% 
  mutate(Probs = predict(reg2, inspections17, type= "response")) %>%
  mutate(predOutcome  = as.factor(ifelse(inspections17$Probs > 0.41 , 1, 0))) %>%
  mutate(incorrect_prediction = ifelse((results_numeric==1 & predOutcome==0), 1, ifelse((results_numeric==0 & predOutcome==1), 1, 0)))
